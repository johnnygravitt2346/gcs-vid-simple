...
class PathResolver:
    ...
    def _setup_paths(self):
        ...
        self.gcs_channels = f"{self.gcs_base}/{self.channels_folder}/{self.channel_id}"
        self.gcs_jobs = f"{self.gcs_base}/jobs"
        
        # New clean content type path
        self.gcs_content_type = f"{self.gcs_channels}/{self.default_content_type}"
        
        # Use a local temp directory for scratch space if not in production
        if os.environ.get("ENV") == "production":
            self.scratch_root = Path("/var/trivia/work")
        else:
            self.scratch_root = Path(tempfile.gettempdir()) / "trivia_work"
    ...
    def content_type_uri(self, content_type: str = None) -> str:
        """Get GCS URI for content type folder."""
        if content_type is None:
            content_type = self.default_content_type
        return f"{self.gcs_channels}/{content_type}"
    
    def dataset_uri(self, version: str, content_type: str = None) -> str:
        """Get GCS URI for a specific dataset version."""
        content_type_path = self.content_type_uri(content_type)
        return f"{content_type_path}/datasets/{version}"
    
    def channel_config_uri(self) -> str:
        """Get GCS URI for channel configuration file."""
        return f"{self.gcs_channels}/channel-config.json"
    
    def assets_uri(self, content_type: str = None) -> str:
        """Get GCS URI for channel assets."""
        content_type_path = self.content_type_uri(content_type)
        return f"{content_type_path}/assets"
    
    def templates_uri(self, content_type: str = None) -> str:
        """Get GCS URI for channel templates."""
        content_type_path = self.content_type_uri(content_type)
        return f"{content_type_path}/templates"
    
    def dev_uri(self, category: str = None) -> str:
        """Get GCS URI for development folder."""
        if category:
            return f"{self.gcs_base}/{self.dev_folder}/{category}"
        return f"{self.gcs_base}/{self.dev_folder}"
    
    def shared_uri(self, resource_type: str = None) -> str:
        """Get GCS URI for shared resources."""
        if resource_type:
            return f"{self.gcs_base}/{self.shared_folder}/{resource_type}"
        return f"{self.gcs_base}/{self.shared_folder}"
    
    def system_uri(self, category: str = None) -> str:
        """Get GCS URI for system folder."""
        if category:
            return f"{self.gcs_base}/{self.system_folder}/{category}"
        return f"{self.gcs_base}/{self.system_folder}"
