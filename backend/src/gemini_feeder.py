...
"""
    ...
    async def _get_channel_dedup_hashes(self, channel_id: str) -> set:
        ...
        try:
            dedup_uri = f"gs://{self.path_resolver.bucket_name}/channels/{channel_id}/dedup/hashes.jsonl"
    ...
    async def _update_channel_dedup_hashes(self, channel_id: str, new_hashes: List[str]) -> None:
        ...
        try:
            ...
            self.cloud_storage.ensure_gcs_directory(f"gs://{self.path_resolver.bucket_name}/channels/{channel_id}/dedup", f"dedup_dir_{channel_id}")
    ...
    async def _publish_dataset(self, questions: List[TriviaQuestion], request: FeederRequest) -> str:
        ...
        dataset_base_uri = f"gs://{self.path_resolver.bucket_name}/datasets/{request.channel_id}/{version}"
    ...
    async def _ensure_unique_version(self, channel_id: str, version: str) -> str:
        ...
        while True:
            dataset_uri = f"gs://{self.path_resolver.bucket_name}/datasets/{channel_id}/{version}"
