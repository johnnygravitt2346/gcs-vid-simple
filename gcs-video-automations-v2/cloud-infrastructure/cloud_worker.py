...
class GPUWorker:
    ...
    def __init__(self, job_leaser: GCSJobLeaser):
        ...
        # Expose client and bucket for convenience
        self.client = job_leaser.client
        self.bucket = job_leaser.bucket
    ...
    def render_video_job(self, job: JobInfo) -> bool:
        """Render a complete video job by invoking the core cloud generator."""
        try:
            ...
            # Determine CSV path; if not present, synthesize from config.json
            csv_blob = self.bucket.blob(f"{job_dir}/questions.csv")
            if not csv_blob.exists(self.client):
                logger.info("questions.csv not found; attempting to convert config.json to CSV")
                ...
                # Build CSV content matching core expectations
                header = ["Question", "OptionA", "OptionB", "OptionC", "OptionD", "Correct Answer"]
                lines = [",".join(header)]
                for q in questions:
                    row = [
                        q.get("question", ""),
                ...
                logger.info("Uploaded synthesized questions.csv")

            gcs_csv_path = f"gs://{GCS_BUCKET}/{job_dir}/questions.csv"

            # Build output path within the bucket
            output_path = f"{FINAL_VIDEOS_PREFIX}/{job.channel}/{job.job_id}/final_video.mp4"

            # Invoke core video generator
            core_job = CoreJobInfo(
                job_id=job.job_id,
                channel=job.channel,
            ...
            final_video_path = core_process_job(core_job)
            ...
            logger.info(f"Job {job.job_id} completed successfully: {final_video_path}")
            return True
